...
### 요약
- PWA/브라우저 알림으로 목표 기반 시작, 남은 시간, 완료 메시지를 보여주도록 추가했습니다.
- 타이머 완료 시 지원 브라우저에서 진동과 짧은 완료음이 재생되도록 보강했습니다.
...
#### 1) 목표 기반 알림
- 집중 세션 시작 시 `뽀모도로 앱 수정 시작, 종료 예정 15:42`처럼 목표 문구를 우선 사용한 알림을 생성합니다.
- 목표 문구가 없으면 `집중`을 기본 문구로 사용합니다.
- 집중 시간이 10분, 5분 이하로 내려갈 때 각각 `10분 남음`, `5분 남음` 알림을 생성합니다.
- 집중 세션 완료 시 `뽀모도로 앱 수정 완료`처럼 목표 문구 기반 완료 알림을 생성합니다.
- 알림 클릭 시 가능한 경우 기존 앱 창을 다시 포커스하도록 서비스워커 클릭 핸들러를 추가했습니다.

#### 2) 완료 피드백
- 타이머 완료 시 `navigator.vibrate`를 지원하는 모바일 브라우저에서 진동을 요청합니다.
- 사용자 시작 동작에서 Web Audio를 준비해두고, 완료 시 짧은 완료음을 재생합니다.
- 브라우저/OS가 진동이나 오디오를 막는 경우 앱 흐름은 그대로 유지되며 조용히 실패합니다.
...
#### 4) 테스트 보강
- `tests/cdp_smoke.py`에 시간 오프셋을 이용한 백그라운드 복귀 보정, 목표 기반 알림, 10분/5분 알림, 완료 진동 피드백 시나리오를 추가했습니다.
- `docs/test-cases.md`를 `TC-14`까지 갱신했습니다.
...
- `src/app/pomodoro/notifications.js`
- `public/sw.js`
- `tests/cdp_smoke.py`
- `docs/test-cases.md`
- `CHANGELOG.md`
